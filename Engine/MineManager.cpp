...
void MineManager::CheckCollision( BulletManager & Bullets, Ship & _Ship )
{
	const int bulletCount = Bullets.GetnBullets();
	Bullet *pBullets = Bullets.GetBullets();

	for( int j = 0; j < nMines; ++j )
	{
		for( int i = 0; i < bulletCount; ++i )
		{
			Bullet &bullet = pBullets[ i ];
			const auto wasHit = CollisionHandler::BulletVsMine( bullet, mine[ j ] );
			if( wasHit )
			{
		...
		const auto wasHit = CollisionHandler::ShipVsMine( _Ship, mine[ j ] );
		if( wasHit )
		{
			if( mine[ j ].IsAlive() )
			{
				_Ship.Damage( mine[ j ].GetDamage() );
				mine[ j ].Detonate();
			}
			else 
				int a = 0;
